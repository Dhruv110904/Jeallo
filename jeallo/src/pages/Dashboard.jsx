              ...
              <div className="space-y-4">
                {recentTasks.length > 0 ? recentTasks.map((task) => (
                  <div key={task.id} className="group flex items-center gap-6 p-6 rounded-[2rem] hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${task.priority === 'critical' || task.priority === 'high' ? 'bg-red-50 text-red-500' : 'bg-slate-50 text-slate-400'
                      }`}>
                    ...
                    <div className="text-right">
                      <span className={`text-[10px] font-black uppercase px-3 py-1 rounded-full ${task.priority === 'critical' ? 'bg-red-500 text-white' :
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-10 animate-in fade-in duration-700">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <h1 className="text-4xl font-black text-slate-900 tracking-tight">
            Welcome back Dhruv, {user?.name?.split(' ')[0]}! 👑
          </h1>
          <p className="text-slate-500 font-medium flex items-center gap-2">
                ...
                return (
                  <div key={task.id} className="group flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-[2rem] hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100">
                    <div className="flex items-center gap-4 flex-1">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${task.priority === 'critical' || task.priority === 'high' ? 'bg-red-50 text-red-500' : 'bg-slate-50 text-slate-400'
                        }`}>
                        <CheckCircle2 className="w-6 h-6" />
                      </div>
                    ...
                    <div className="flex items-center justify-between md:justify-end gap-3 self-stretch md:self-auto border-t md:border-t-0 pt-3 md:pt-0 border-slate-50">
                      <span className={`text-[9px] font-black uppercase px-2.5 py-1 rounded-md shrink-0 ${task.status === 'Done' ? 'bg-done-green/10 text-done-green border border-done-green/20' : 'bg-slate-100 text-slate-500 border border-slate-200/50'
                        }`}>
                        {task.status}
                      </span>
                      <span className={`text-[9px] font-black uppercase px-2.5 py-1 rounded-full shrink-0 ${task.priority === 'critical' ? 'bg-red-500 text-white shadow-sm shadow-red-500/20' :
